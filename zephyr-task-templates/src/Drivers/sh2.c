...
LOG_MODULE_REGISTER(sh2_c, LOG_LEVEL_INF);
...
static void sensorhubAdvertHdlr(void *cookie, uint8_t tag, uint8_t len, uint8_t *value)
{
	...
	LOG_DBG("sensorhubAdvertHdlr CALLBACK : tag=%d executableChan=%d controlChan=%d", tag, pSh2->executableChan, pSh2->controlChan);
    ...
    switch (tag) {
        ...
        case 0:
        {
			...
			LOG_DBG("sensorhubAdvertHdlr: SHTP layer can give us our channel numbers");
			LOG_DBG("executableChan=%d controlChan=%d", pSh2->executableChan, pSh2->controlChan);
...
static int setSensorConfigStart(sh2_t *pSh2)
{
	...
	if(pSh2->controlChan == 0)
		LOG_DBG("%s: Channel %d",__func__, pSh2->controlChan);
	else
		LOG_DBG("%s: Channel %d",__func__, pSh2->controlChan);
	...
	LOG_DBG("%s: Sending SENSORHUB_SET_FEATURE_CMD req Channel %d",__func__, pSh2->controlChan);
	LOG_HEXDUMP_INF(&req , sizeof(req), "req=");
             ...
             sh2_EventCallback_t *eventCallback, void *eventCookie)
{
	...
	// @fecarraustewart Forcing Bad Channel Reported
	// pSh2->resetComplete = true;  // will go true after reset response from SH.
	// pSh2->controlChan = 0;  // An invalid value since we don't know yet.
