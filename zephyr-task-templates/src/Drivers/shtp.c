...
LOG_MODULE_REGISTER(shtp_c, LOG_LEVEL_INF);
...
static int txProcess(shtp_t *pShtp, uint8_t chan, const uint8_t* pData, uint32_t len)
{
    ...
    while (remaining > 0) {
        ...
        if (status < 0)
        {
			...
			LOG_ERR("txProcess: Error, throw away this cargo");
            pShtp->txDiscards++;
            return status;
        }
...
static void rxAssemble(shtp_t *pShtp, uint8_t *in, uint16_t len, uint32_t t_us)
{
    ...
    if (len < SHTP_HDR_LEN) {
		LOG_DBG("rxAssemble: len < SHTP_HDR_LEN. Discards invalid short fragments");
    ...
    if (payloadLen < SHTP_HDR_LEN) {
	  LOG_DBG("rxAssemble: payloadLen < SHTP_HDR_LEN. Short fragment.");
      pShtp->shortFragments++;
    ...
    if ((chan >= SH2_MAX_CHANS) ||
        ...
        // Invalid channel id.
		LOG_DBG("rxAssemble: Invalid channel id. SHTP_BAD_RX_CHAN.");
        pShtp->badRxChan++;
    ...
    if (pShtp->inRemaining == 0) {
        if (payloadLen > sizeof(pShtp->inPayload)) {
			...
			LOG_DBG("rxAssemble: This payload won't fit! Discard it. SHTP_TOO_LARGE_PAYLOADS.");
    ...
    if (pShtp->inRemaining == 0) {
		LOG_DBG("rxAssemble: Whole payload received. Deliver it to channel listener = %d.", chan);
        // Call callback if there is one.
        if (pShtp->chan[chan].callback != 0) {
			LOG_DBG("rxAssemble: Found callback for channel listener = %d.", chan);

            pShtp->chan[chan].callback(pShtp->chan[chan].cookie,
                                       pShtp->inPayload, pShtp->inCursor,
                                       pShtp->inTimestamp);
        } else {
			LOG_DBG("rxAssemble: No callback for channel listener = %d.", chan);
			LOG_HEXDUMP_DBG(pShtp, sizeof(shtp_t), "pShtp: ");
		}
    }
...
void *shtp_open(sh2_Hal_t *pHal)
{
    ...
    if (pHal == 0) {
        // Error
		LOG_ERR("shtp_open: params Error");
        return 0;
    }
    ...
    if (pShtp == 0) {
		...
		LOG_ERR("shtp_open: No instances available, return error");
        return 0;
    }
                    ...
                    const char * appName, const char * chanName)
{
	...
	LOG_DBG("appName: %s, chanName: %s\n", appName, chanName);
	LOG_HEXDUMP_INF(pInstance, sizeof(shtp_t), "pInstance: ");

    // Determine GUID for this appname
    for (int n = 0; n < SH2_MAX_APPS; n++) {
		LOG_DBG("Looking for match appName: %s, chanName: %s. Channel = %d\n", pShtp->app[n].appName, pShtp->chan[n].chanName, pShtp->app[n].guid);
              ...
              uint8_t channel, const uint8_t *payload, uint16_t len)
{
    ...
    if (len > pShtp->outMaxPayload) {
		LOG_ERR("shtp_send: Payload too large %d\n", len);
        return SH2_ERR_BAD_PARAM;
    }
    if (channel >= SH2_MAX_CHANS) {
        pShtp->badTxChan++;
		LOG_ERR("shtp_send: Bad channel %d\n", channel);
        return SH2_ERR_BAD_PARAM;
    }
...
void shtp_service(void *pInstance)
{
    ...
    if (pShtp->advertPhase == ADVERT_NEEDED) {
        ...
        if (status != SH2_OK) {
			...
			LOG_ERR("Advert request failed. Go back to needing one");
            pShtp->advertPhase = ADVERT_NEEDED;
        }
    }
    ...
    if (len) {
		...
		LOG_HEXDUMP_DBG(pShtp->inTransfer, sizeof(pShtp->inTransfer), "Read data: ");
