...
LOG_MODULE_REGISTER(buzzer_output_drv, LOG_LEVEL_DBG);
...
static const struct pwm_dt_spec sBuzzer = PWM_DT_SPEC_GET(DT_NODELABEL(fading_leds));
...
static struct note_duration double_beep_song[] = {
...
void buzzer_thread(void)
{
	...
	case 1:
		...
		for (int i = 0; i < FUNKYTOWN_NOTES; i++) {
			if (funkytown_song[i].note < 10) {
				...
				zpp::this_thread::sleep_for(std::chrono::milliseconds(funkytown_song[i].duration));
			} else {
	...
	case 2:
		...
		for (int i = 0; i < MARIO_NOTES; i++) {
			if (mario_song[i].note < 10) {
				/* Low frequency notes represent a 'pause' */
				pwm_set_pulse_dt(&sBuzzer, 0);
				zpp::this_thread::sleep_for(std::chrono::milliseconds(mario_song[i].duration));
			} else {
	...
	case 3:
		...
		for (int i = 0; i < (sizeof(golioth_song) / sizeof(golioth_song[1])); i++) {
			if (golioth_song[i].note < 10) {
				/* Low frequency notes represent a 'pause' */
				pwm_set_pulse_dt(&sBuzzer, 0);
				zpp::this_thread::sleep_for(std::chrono::milliseconds(golioth_song[i].duration));
			} else {
	...
	case 4:
		LOG_DBG("double_beep");
		for (int i = 0; i < (sizeof(double_beep_song) / sizeof(double_beep_song[1])); i++) {
			if (double_beep_song[i].note < 10) {
				/* Low frequency notes represent a 'pause' */
				pwm_set_pulse_dt(&sBuzzer, 0);
				zpp::this_thread::sleep_for(std::chrono::milliseconds(double_beep_song[i].duration));
			} else {
				pwm_set_dt(&sBuzzer, PWM_HZ(double_beep_song[i].note),
						PWM_HZ((double_beep_song[i].note)) / 2);
				zpp::this_thread::sleep_for(std::chrono::milliseconds(double_beep_song[i].duration));
			}
		}
		break;
