...
LOG_MODULE_REGISTER(BNO085_Driver, LOG_LEVEL_INF);
...
bool BNO085::begin(TwoWire *wirePort)
{
	...
	while(status != SH2_OK)
	{
		...
		if (status != SH2_OK) {
			LOG_ERR("status != SH2_OK at sh2_open");
		}
		RTOS::Hal::Delay(10);
	}
	...
	while(status != SH2_OK)
	{
		...
		status = 0;
		if (status != SH2_OK) {
			LOG_ERR("status != SH2_OK at sh2_getProdIds");
		}
		RTOS::Hal::Delay(10);
	}
...
bool BNO085::enableReport(sh2_SensorId_t sensorId, uint32_t interval_us,
  ...
  if (status != SH2_OK) {
	LOG_ERR("Enabling Sensor[%d] returned status != SH2_OK.", sensorId); 
    return false;
  }
...
static int i2chal_open(sh2_Hal_t *self) {
	LOG_DBG("I2C HAL open now DOES NOT RESET DEVICE.");

//   uint8_t softreset_pkt[] = {5, 0, 1, 0, 1};
//   bool success = false;
//   for (uint8_t attempts = 0; attempts < 5; attempts++) {
//     if (I2CWrite(_address,softreset_pkt, 5)) {
// 		LOG_DBG("I2C HAL open Success");
//       success = true;
//       break;
//     }
//     RTOS::Hal::Delay(30);
...
static uint32_t hal_getTimeUs(sh2_Hal_t *self) {
  uint32_t t = (uint32_t)k_uptime_get() * 1000;
  LOG_DBG("I2C HAL get time: %d\n", t);
  return t;
}
...
static void hal_callback(void *cookie, sh2_AsyncEvent_t *pEvent) {
  ...
  if (pEvent->eventId == SH2_RESET) {
	...
	uint8_t msg[] = {0xA5, SH2_RESET, 0x0 , 0x0, 0x0};
	Service::IMU::Send(msg);
...
static void sensorHandler(void *cookie, sh2_SensorEvent_t *event) {
  ...
  if (rc != SH2_OK) {
    LOG_ERR("BNO085 - Error decoding sensor event");
    _sensor_value->timestamp = 0;
    return;
  }
}
...
bool I2CWrite(uint8_t add, uint8_t *buffer, size_t size) {
  ...
  if (Wire.write(buffer, size) != size) {
	LOG_ERR("I2CWrite: Wire.write failed");
...
bool BNO085::softReset(void)
{
  ...
  if (status != SH2_OK) {
	LOG_ERR("status != SH2_OK at sh2_devReset");
    return false;
  }
